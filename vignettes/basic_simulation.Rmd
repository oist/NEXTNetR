...
We start with loading the *NEXTNetR* package. If the package is not already installed, see the [website](https:://oist.github.com/nextnetr) for installation instructions.
...
The object `nw` now represents a specific network. We can inspect the network using the functions listed in `help(network_functions)`. For example, we can query the number of neighbours of node 371 with
...
and start the simulation, telling it to run until either time 100 or until 90,000 nodes have been infected.
