...
The following R script sets up and runs a simulation on an Erdős–Rényi network with lognormally distributed transmission time (i.e. the generation time between subsequent infections) and lognormally distribured reset time (the time until an infected node becomes susecptible again).
